	...
	function dataBill(res) {
		...
		res.forEach(function(item) {
			var tbody_tr = `<tr class="item_bill">
								<td>${item.tenMonAn}</td>	
								<td style="text-align: center"><span><button data-item='${JSON.stringify(item)}' class="btn_add">+</button>${item.soLuong}<button data-item='${JSON.stringify(item)}' class="btn_minus">-</button></span></td>
