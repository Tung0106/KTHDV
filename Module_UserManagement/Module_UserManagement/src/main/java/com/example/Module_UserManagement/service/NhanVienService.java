...
public class NhanVienService {
	...
	public ResponseEntity<Object> addEmployee(NhanVien nhanVien) {
		...
		if (repo.findByTaiKhoan(nhanVien.getTaiKhoan()).isEmpty()) {
			nhanVien.setThoiGianTao();
			return new ResponseEntity<>(repo.save(nhanVien), HttpStatus.CREATED);
		}

		return new ResponseEntity<>("Nhân viên đã tồn tại", HttpStatus.CONFLICT);
	}

	public ResponseEntity<Object> login(NhanVien nhanVienReq) {
		Optional<NhanVien> nhanVien = repo.findByTaiKhoanAndMatKhau(nhanVienReq.getTaiKhoan(), nhanVienReq.getMatKhau());
